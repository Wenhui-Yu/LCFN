...
from package.utils.train_model import train_model
from package.utils.print_save import print_params, save_params
from package.params.get_hyperparameters import get_hyperparameter
import tensorflow as tf
import numpy as np
import random as rd
import time

def tuning(path_excel_dir, para, data, lr, lamda, min_num_coarse, max_num_coarse, min_num_fine, max_num_fine):
    ## tuning settings
    x_cen, y_cen = 1, 1
    ...
    for i in range(2):
        for x_curr, y_curr in [[x_cen, y_cen], [x_cen - 1, y_cen], [x_cen, y_cen - 1], [x_cen + 1, y_cen], [x_cen, y_cen + 1]]:
            if (num_matrix[x_curr, y_curr] < min_num_coarse or (x_curr == x_cen and y_curr == y_cen)) and (num_matrix[x_curr, y_curr] < 0.5 or score_matrix[x_curr, y_curr] >= 0.7 * score_matrix.max()):
                ...
                if para["MODEL"] not in ['SCF', 'LightGCN']: tf.reset_default_graph()
