...
def train_model(para, path_excel):
    [_,MODEL,LR,LAMDA,LAYER,EMB_DIM,FREQUENCY_USER, FREQUENCY_ITEM,FREQUENCY,KEEP_RATE,
    ...
    pre_train_bases_path = DIR+'propagation_embeddings'+str(FREQUENCY)+'.json'    # to pretrain latent factors for random walk
    ...
    if MODEL == 'SGNN': pre_train_bases = read_bases(pre_train_bases_path, FREQUENCY, FREQUENCY)
    ## Define the model
    if MODEL == 'BPR':
    ...
    if MODEL == 'LCFN':
        model = model_LCFN(layer=LAYER, n_users=user_num, n_items=item_num, emb_dim=EMB_DIM, 
                           ...
                           pre_train_latent_factor=pre_train_feature, if_pretrain=IF_PRETRAIN)
    if MODEL == 'SGNN':
        model = model_SGNN(layer=LAYER, n_users=user_num, n_items=item_num, emb_dim=EMB_DIM, frequency=FREQUENCY,
                           lr=LR, lamda=LAMDA, optimization=OPTIMIZATION, pre_train_latent_factor=pre_train_feature,
                           pre_train_propagation_embeddings=pre_train_bases, if_pretrain=IF_PRETRAIN)
        ...
        for batch_num in range(len(batches)-1):
            ...
            _, loss = sess.run([model.updates, model.loss],
                               feed_dict={model.users: train_batch_data[:,0],
                                          ...
                                          model.keep_prob: np.array(KEEP_RATE)})
