...
#             embeddings = tf.nn.sigmoid(tf.matmul(embeddings_1, self.filters_1[k]) + tf.matmul(embeddings_2, self.filters_2[k]))
        ...
        self.pos_scores = tf.reduce_sum(tf.multiply(self.u_embeddings_drop, self.pos_i_embeddings_drop), axis=1)
        self.neg_scores = tf.reduce_sum(tf.multiply(self.u_embeddings_drop, self.neg_i_embeddings_drop), axis=1)
        self.diff = self.pos_scores - self.neg_scores
        self.sig_diff = tf.nn.sigmoid(self.diff)
        self.maxi = tf.log(self.sig_diff+0.1*10)
        self.loss = tf.negative(tf.reduce_sum(self.maxi))
    ...
    def create_bpr_loss(self, users, pos_items, neg_items):
        ...
        maxi = tf.log(tf.nn.sigmoid(pos_scores - neg_scores)+0.1*10)
